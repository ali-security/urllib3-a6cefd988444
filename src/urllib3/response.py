...
try:
    ...
    try:
        ...
        class _ZstdBlockSplitter:
            ...
            returns an object whose ``decompress()`` takes no
            ``max_length`` argument, so it decompresses everything it is
            fed. Since a Zstandard block expands to at most 128 KiB,
            feeding the decompressor one block at a time keeps the amount
            of data decompressed in a single step bounded, and
            decompression can be stopped as soon as enough data has been
            produced.

            Splitting is advisory only. The decompressor accepts input
            split at arbitrary boundaries, so a mismatch between this
            splitter and the actual stream can affect how much data is
            decompressed at once, but never the decompressed data itself.
            """
            ...
            def _parse_header(self) -> None:
                ...
                if self._state == self._MAGIC:
                    ...
                    if magic == _ZSTD_MAGIC_NUMBER:
                        ...
                        _ZSTD_SKIPPABLE_MAGIC_MIN <= magic <= _ZSTD_SKIPPABLE_MAGIC_MAX
                    ):
